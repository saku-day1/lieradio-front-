...
- 機能変更・バグ修正は必ず **`feature/*` ブランチ**を作成してから作業する
- `feature/*` ブランチへのコミット・プッシュはユーザーの確認なしで自動的に行う
- **mainブランチへの直接pushは禁止**（ただし **CLAUDE.md の変更のみ**の場合は確認不要で直接コミット可）
...
実装に着手する前に必ず以下を確認・説明すること。

- **責務の明示**: 変更対象のファイルについて「なぜそのファイルを変更するか」「そのモジュールの責務は何か」「他レイヤーへの責務漏れがないか」を説明する
- **仕様確認優先**: 仕様が曖昧な場合、推測で実装せずユーザーに確認を取る
- **危険変更は計画先出し**: 複数ファイルにまたがる変更・既存仕様の変更・破壊的変更を行う場合は、実装計画を先に提示してから着手する
- **既存仕様変更の理由説明**: 既存の動作・仕様を変える場合は、変更理由を明記する
- **DB・JSON・キャッシュ削除処理は必ず確認**: `data/*.json` の上書き、Redis キャッシュの削除・無効化、スキーマ変更を伴う処理はユーザーの確認を取ってから実行する
- **必要に応じて skills を参照**: 用途に応じて以下を使うこと
  - `/safe` — 実装規律チェック（大量変更・推測実装・責務違反の検知）
  - `/issue` — スコープ分割・Issue 切り出し・MVP 判断
  - `/review` — マージ前の最終チェック（セキュリティ・責務・法的）
  - `/ux` — UI/UX 方針の確認（ライト層優先・1クリック到達・モバイル優先）
  - `/actions` — GitHub Actions 変更時のチェック（secrets・Cron・version 固定）
  - `/vercel` — Vercel 運用チェック（Serverless 制約・キャッシュ戦略・RateLimit）
  - `/legal` — 公開前の法的チェック（転載・YouTube 規約・権利表記）
  - `/sync` — データ同期設計チェック（差分検知・冪等性・fail-safe・rollback）
...
### 法的
- **コンテンツ本文の転載・ホスティング禁止**: 動画・音声・画像など著作物のファイルを `data/` 等にコミットしていないか。番組・放送の内容テキストも転載不可。検索・索引・導線（タイトル・URLリンク）の範囲に留めること（YouTubeへのリンクは可）
...
- **再利用性・保守性・構造整理を優先する**: 短期的な実装速度よりも、後から変更しやすい構造を選ぶ。不要なリファクタは禁止だが、設計的に正しい分離は積極的に行う。
- **既存命名に合わせる**: 変数名・型名・キャッシュキー・JSON フィールド名は既存の命名規則に従う。新しい概念（レイヤー・抽象・用語）を不必要に増やさない。
- **大量通信・全件同期は禁止**: API呼び出しや Redis 書き込みは差分更新を優先する。全件再取得・全件上書きが必要な場合は理由を説明してからにすること。
- **不要な抽象化をしない**: 3箇所以上で使われるまで共通化しない。1〜2箇所の類似コードは重複を許容する。
...
- **UI変更時は実装前に変更前後の差分を文章で説明する**。ユーザーが視覚的に確認できるよう「現状→変更後」の形で説明してから実装に着手する。
