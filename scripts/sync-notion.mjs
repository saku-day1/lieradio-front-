...
async function upsertEpisode(episode, notionIndex) {
  ...
  if (existing) {
    ...
    if ([81, 138].includes(episode.broadcastNumber)) {
      console.log(`[debug ${episode.broadcastNumber}] Notion:`, existing.fingerprint);
      console.log(`[debug ${episode.broadcastNumber}] New:   `, newFingerprint);
    }
    if (!DRY_RUN) {
      if (USE_FETCH_FALLBACK) {
